 ...
 * Limitations:
 * <UL>
 * <LI>Current implementation does not protect against changing
 * direction and backing out of the block
 * <LI>Should track speed at time of stop and restore it on restart (or
...
public class StoppingBlock {
    ...
    void handleBlockChange(java.beans.PropertyChangeEvent e) {
        ...
        if (e.getPropertyName().equals("state") && e.getNewValue().equals(Integer.valueOf(Block.OCCUPIED)) ) {
            ...
            if (log.isDebugEnabled()) log.debug("Block "+block.getSystemName()+" occupied with "+val);
        
            if ( val == SignalHead.RED) doStop();
            if ( val == SignalHead.YELLOW) doSlow();
        }
    }
    ...
    void handleSignalChange(java.beans.PropertyChangeEvent e) {
        ...
        if ( (block.getValue() != null) && block.getState() == (Block.OCCUPIED)) {
            ...
            if (direction != block.getDirection()) return;  // not interesting
        
            int val = fastestAppearance();
            if (log.isDebugEnabled()) log.debug("Block "+block.getSystemName()+" signal change to "+val);
        
            if ( val == SignalHead.YELLOW) doSlow();
            if ( val == SignalHead.GREEN) doRestart();
        }
    }
