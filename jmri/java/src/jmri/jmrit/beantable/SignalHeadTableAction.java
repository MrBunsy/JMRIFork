...
public class SignalHeadTableAction extends AbstractTableAction {
    ...
    protected void addPressed(ActionEvent e) {
        if (addFrame==null) {
            ...
            v1Panel = new JPanel(); v1Panel.setLayout(new FlowLayout());
            v1Panel.add(ato1);
            v1Panel.add(to1);
            v1Panel.add(s1Box);
            v1Panel.add(msaBox);
            ...
            v2Panel = new JPanel(); v2Panel.setLayout(defaultFlow);
            v2Panel.add(to2);
            v2Panel.add(s2Box);
            v2Panel.add(s2aBox);
            v2Panel.add(mstBox);
            ...
            v3Panel = new JPanel(); v3Panel.setLayout(defaultFlow);
            v3Panel.add(to3);
            v3Panel.add(s3Box);
            v3Panel.add(s3aBox);
            ...
            v4Panel = new JPanel(); v4Panel.setLayout(defaultFlow);
            v4Panel.add(to4);
            v4Panel.add(s4Box);
            v4Panel.setBorder(v4Border);
            panelCentre.add(v4Panel);

            v5Panel = new JPanel(); v5Panel.setLayout(defaultFlow);
            v5Panel.add(to5);
            v5Panel.add(s5Box);
            v5Panel.setBorder(v5Border);
            panelCentre.add(v5Panel);

            v6Panel = new JPanel(); v6Panel.setLayout(defaultFlow);
            v6Panel.add(to6);
            v6Panel.add(s6Box);
            v6Panel.setBorder(v6Border);
            panelCentre.add(v6Panel);

            v7Panel = new JPanel(); v7Panel.setLayout(defaultFlow);
            //v7Panel.add(v7Label);
            v7Panel.add(to7);
            v7Panel.add(s7Box);
            ...
            p = new JPanel(); p.setLayout(defaultFlow);
            p.add(vtLabel);
            p.add(stBox);
            panelCentre.add(p);
    ...
    void okPressed(ActionEvent e) {
        ...
        if (se8c4Aspect.equals(typeBox.getSelectedItem())) {
            ...
            if (checkBeforeCreating(systemName.getText())) {
                ...
                if (t1==null) addTurnoutMessage(v1Border.getTitle(), to1.getDisplayName());
                if (t2==null) addTurnoutMessage(v2Border.getTitle(), to2.getDisplayName());
                if (t3==null) addTurnoutMessage(v3Border.getTitle(), to3.getDisplayName());
                if (t4==null) addTurnoutMessage(v4Border.getTitle(), to4.getDisplayName());
            ...
            if (checkBeforeCreating(systemName.getText())) {
                ...
                if (t3==null) addTurnoutMessage(v3Border.getTitle(), to3.getDisplayName());
                if (t4==null) addTurnoutMessage(v4Border.getTitle(), to4.getDisplayName());
                if (t5==null) addTurnoutMessage(v5Border.getTitle(), to5.getDisplayName());
                if (t6==null) addTurnoutMessage(v6Border.getTitle(), to6.getDisplayName());
                if (t7==null) addTurnoutMessage(v7Border.getTitle(), to7.getDisplayName());
	...
	void handleMergSignalDriverOkPressed() {
        ...
        if (checkBeforeCreating(systemName.getText())) {
            switch(ukSignalAspectsFromBox(msaBox)){
                case 4: t3 = getTurnoutFromPanel(to5, "SignalHead:" + systemName.getText() + ":Input3");
                        if (t3==null) {
                        ...
                        // fall through
                case 3: t2 = getTurnoutFromPanel(to4, "SignalHead:" + systemName.getText() + ":Input2");
                        if (t2==null) {
                        ...
                        // fall through
                case 2: t1 = getTurnoutFromPanel(to3, "SignalHead:" + systemName.getText() + ":Input1");
                        if (t1==null) {
	...
	void makeEditSignalWindow() {
		...
		if (editFrame == null) {
            ...
            ev1Panel = new JPanel(); ev1Panel.setLayout(defaultFlow);
            ev1Panel.add(eto1);
            ev1Panel.add(es1Box);
            ev1Panel.add(emsaBox);
            ev1Panel.setBorder(ev1Border);
            panelCentre.add(ev1Panel);
            ev2Panel = new JPanel(); ev2Panel.setLayout(defaultFlow);
            ...
            ev4Panel = new JPanel(); ev4Panel.setLayout(defaultFlow);

            ev4Panel.add(eto4);
            ev4Panel.add(es4Box);
            ev4Panel.setBorder(ev4Border);
            panelCentre.add(ev4Panel);
            ev5Panel = new JPanel(); ev5Panel.setLayout(defaultFlow);

            ev5Panel.add(eto5);
            ev5Panel.add(es5Box);
            ev5Panel.setBorder(ev5Border);
            panelCentre.add(ev5Panel);
            ev6Panel = new JPanel(); ev6Panel.setLayout(defaultFlow);

            ev6Panel.add(eto6);
            ev6Panel.add(es6Box);
            ev6Panel.setBorder(ev6Border);
            panelCentre.add(ev6Panel);
            ev7Panel = new JPanel(); ev7Panel.setLayout(defaultFlow);
	...
	void updatePressed(ActionEvent e) {
		...
		if (className.equals("jmri.implementation.QuadOutputSignalHead")) {
			...
			else ((TripleOutputSignalHead)curS).setGreen(nbhm.getNamedBeanHandle(eto1.getDisplayName(),t1));
			...
			else ((TripleOutputSignalHead)curS).setBlue(nbhm.getNamedBeanHandle(eto2.getDisplayName(),t2));
			...
			else ((TripleOutputSignalHead)curS).setRed(nbhm.getNamedBeanHandle(eto3.getDisplayName(),t3));
		}
		else if (className.equals("jmri.implementation.DoubleTurnoutSignalHead")) {
...
//          ((jmri.AcelaSignalHead)curS).setDarkState(signalheadTypeFromBox(estBox));    
            ...
            switch(ukSignalAspectsFromBox(emsaBox)){
                case 4: Turnout t3 = updateTurnoutFromPanel(eto5, "SignalHead:" + eSysNameLabel.getText() + ":Input3", ((jmri.implementation.MergSD2SignalHead)curS).getInput3().getBean(), ev5Border.getTitle());
                        if (t3==null) {
                            return;
                            }
                        else ((jmri.implementation.MergSD2SignalHead)curS).setInput3(nbhm.getNamedBeanHandle(eto5.getDisplayName(),t3));
                        // fall through
                case 3: Turnout t2 = updateTurnoutFromPanel(eto4, "SignalHead:" + eSysNameLabel.getText() + ":Input2", ((jmri.implementation.MergSD2SignalHead)curS).getInput2().getBean(), ev4Border.getTitle());
                        if (t2==null) {
                            return;
                            }
                        else ((jmri.implementation.MergSD2SignalHead)curS).setInput2(nbhm.getNamedBeanHandle(eto4.getDisplayName(),t2));
                        // fall through
                case 2: Turnout t1 = updateTurnoutFromPanel(eto3, "SignalHead:" + eSysNameLabel.getText() + ":Input1", ((jmri.implementation.MergSD2SignalHead)curS).getInput1().getBean(), ev3Border.getTitle());
                        if (t1==null) {
                            return;
                            }
